...
formatWithList  ::  (Text.Printf.PrintfType a, Text.Printf.PrintfArg t)
                =>  String  ->  [t]
                ->  Maybe a
formatWithList  text vals
    ...
    num = c 0 text
    c n ('%':'s':m) = c (n+1) m ; c n ('%':'v':m) = c (n+1) m ; c n (_:m) = c n m ; c n [] = n
...
multiLinedChunksByIndent
    = multiLinedChunksBy$ const.doesntbeginchunk where
        doesntbeginchunk "" = True ; doesntbeginchunk (c:_) = Data.Char.isSpace c
...
trimSpaceOr ::  [Char]  ->  String
            ->  String
trimSpaceOr dropitems
    = Lst.trim ( Data.Char.isSpace |.| Lst.elemIn dropitems )
