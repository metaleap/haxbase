...
isInfixed ::  T.Text  ->  T.Text  ->  Bool
isInfixed = flip T.isInfixOf

isPrefixed ::  T.Text  ->  T.Text  ->  Bool
isPrefixed = flip T.isPrefixOf

isSuffixed ::  T.Text  ->  T.Text  ->  Bool
isSuffixed = flip T.isSuffixOf



joined ::  T.Text  ->  [T.Text]  ->  T.Text
joined = T.intercalate



multiLinedChunksBy ::  (T.Text->T.Text->Bool)  ->  T.Text  ->  [T.Text]
multiLinedChunksBy doesntbeginchunk =
    (T.unlines <$>) . Lst.groupBy doesntbeginchunk . T.lines

multiLinedChunksByIndent ::  T.Text  ->  [T.Text]
